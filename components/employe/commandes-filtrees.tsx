...
export function CommandesFiltrees({ commandes }: CommandesFiltreesProps) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <fieldset className="grid gap-4 rounded-2xl border border-border bg-background p-6 shadow-card sm:grid-cols-2">
        <legend className="sr-only">Filtrer les commandes</legend>
      ...
      <div aria-live="polite" className="text-sm text-muted-foreground">
        {commandesFiltrees.length} commande{commandesFiltrees.length > 1 ? 's' : ''} correspond
        {commandesFiltrees.length > 1 ? 'ent' : ''} aux critères
      </div>
